...
class LocalPlanner:
    ...
    def choose_goal_cell(self, ego_position, ego_heading, offset = 30.0):
        ...
        while True:
        # while :
            print(f"Trying to refine goal index: {refined_goal_index}, tries: {tries}")
            local_goal = global_2_local_coord_point(self._global_path[refined_goal_index], ego_position, ego_heading,
        ...
        # 4. 将目标点转换为局部坐标系中的单元格
        goal_cell = global_2_local_coord_point(self._global_path[refined_goal_index], ego_position, ego_heading, 
                                                   self._resolution, self._grid_center_x, self._grid_center_y)
    ...
    def find_local_path(self, world, carla_map, ego_position, ego_heading):
        ...
        self._occupancy_grid.save_test_case((start_cell.x, start_cell.y), (goal_cell.x, goal_cell.y))
    ...
    def find_lookahead_point(self, world, carla_map, ego_position, ego_heading, lookahead_len = 1.0):
        ...
        print(f"Warning: No lookahead point found with dist={lookahead_len}, returning last point in local path.")
