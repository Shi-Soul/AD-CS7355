import sys
# sys.path.append("/home/carla/PythonAPI")
...
class VehiclePIDController():
    ...
    def run_step_transform(self, target_speed, transform):
        """
        Execute one step of control invoking both lateral and longitudinal
        ...
        acceleration = self._lon_controller.run_step(target_speed)
        current_steering = self._lat_controller.run_step_transform(transform)
        control = carla.VehicleControl()
        if acceleration >= 0.0:
            control.throttle = min(acceleration, self.max_throt)
            control.brake = 0.0
...
class PIDLateralController():
    ...
    def run_step_transform(self, transform):
        """
        Execute one step of lateral control to steer
        the vehicle towards a certain waypoin.
            ...
            +1 maximum steering to right
        """
        return self._pid_control_transform(transform, self._vehicle.get_transform())
    ...
    def _pid_control_transform(self, transform, vehicle_transform):
        """
        Estimate the steering angle of the vehicle based on the PID equations
...
class Controller():
    ...
    def run_step(self, target_location, target_rotation):
        return self.controller.run_step_transform(self.target_speed, carla.Transform(target_location, target_rotation))