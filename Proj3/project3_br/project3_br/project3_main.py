...
def control_car(args):
    ...
    try:
        ...
        if args.debug:
            print(f"已读取车辆状态，共{len(vehicle_states)}个车辆")
        ...
        if args.debug:
            print(f"其他车辆已生成，共{len(parked_vehicles)}辆")
        sim_world.tick()
        if args.debug:
            print("车辆生成完成")
        ...
        if args.debug:
            print(f"已读取全局路径，共{len(gloabl_path)}个点")
        ...
        while True:
            ...
            # 获取前视点或控制用的未来路径
            if args.debug:
                print(f"当前位置: ({ego_position.x:.2f}, {ego_position.y:.2f}), 速度: {ego_speed:.2f} m/s")
            
            if args.debug:
                print("正在计算前视点...")
            look_ahead_point = local_path_planner_.find_lookahead_point(sim_world, carla_map, ego_position, ego_heading)
            if args.debug:
                print(f"前视点坐标: ({look_ahead_point[0]:.2f}, {look_ahead_point[1]:.2f}), 角度: {look_ahead_point[2]:.2f}")
            ...
            if args.debug:
                print(f"控制命令: 油门={control.throttle:.2f}, 转向={control.steer:.2f}, 刹车={control.brake:.2f}")
